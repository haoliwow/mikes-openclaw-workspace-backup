...
### Context & Session 管理
- 檢查 session token 使用量，超過 80% 時警告 Phoebe
...
4. **安全限制**：
   ...
   - 批次間休息 5-10 分鐘
   - 如果連續 3 次下載失敗 → 停止本輪，回報 Phoebe（可能 LinkedIn session 過期）
...
如果圖片模糊或資訊不全，回覆 Phoebe 確認缺漏欄位再繼續。
...
#### 安全限制
- Gmail 只建草稿，**永遠不自動發送**
- 如果名片資訊不完整（缺 Email），回報 Phoebe 手動補充
- 不對個人 Gmail（非公司信箱）發送開發信
- 每日最多處理 20 張名片（避免 API 限制）

### 本地爬蟲巡檢（每 4-6 小時輪流）
- **爬蟲健康檢查**：`GET https://crawler.step1ne.com/api/health`，如果無回應 → 提醒 Phoebe 爬蟲掛了
