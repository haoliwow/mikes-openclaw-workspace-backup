...
**開發信必備條件（2026-03-19 Phoebe 強調 - 必須遵守）：**

1. ✅ **簽名：JIM（獵頭帳號）** — 統一用 Jim，不用 Mike、其他名字
2. ✅ **自我介紹要具體** — 「獵頭 Jim」而非通用稱呼
3. ✅ **認可背景** — 具體提到他的技能/成就（引用履歷）
4. ❌ **不曝露牌**（第一階段） — **不直接說公司名、職位、薪資、地點**
   - ❌ 錯誤：「仁大資訊 DBA 職位，薪資 60-90k，新北市五股」
   - ✅ 正確：「我最近在幫一家快速成長的系統整合公司找人」
5. ✅ **機會邀請要直接** — 「聊聊一些工作的機會」而非虛偽問「你想什麼」
6. ✅ **避免廢話** — 直接給誘餌（工作機會），不要長篇大論

**核心邏輯（2026-03-12 Phoebe 核心哲學）：**
- 第一階段：讓候選人被「看到」而有回應欲望（不是被推銷）
- 第二階段：在對話中推敲他的真實想法
- 第三階段：才推薦最合適職位並提及公司背景、薪資等細節
...
## 📋 Step1ne 匯入規則（2026-03-19 Phoebe 確立 - 必遵）
...
1. 妳提供履歷 PDF
2. 我分析 → 找適合職缺
3. **有適合職缺** → 我寫開發信 → 妳寄出 → 我匯入「已聯繫」
4. **沒適合職缺或有其他原因不寄** → 我匯入「備選人才」

**避免的錯誤**：
- ❌ 我說「無適合職缺，建議備選」而不匯入 ← **這是錯的**
- ❌ 等妳決定要不要匯入 ← **應該先匯再等指示**
- ❌ 「給我開發信的我都寄了」= 自動轉為「已聯繫」
...
### 執行規則（Phoebe強調）
1. **第一次就對** — 不要「確認」、不要「檢查」、不要「多嘴」
2. **妳給格式 = 我按格式做** — 直接執行，完
3. **不需要過程敘述** — 就是做，然後報結果
4. **簡潔不廢話** — 用符號、用數字、用重點

### 工作內容回寫
- 妳給數據 → 我按格式貼進每日會報群（-1002997693797, thread_id: 526）
- 直接POST，無需確認

### 備份習慣
- 每次改動 → git commit（描述清楚做了什麼）
- 定期更新MEMORY.md（重要決策、格式規則、核心學習）
